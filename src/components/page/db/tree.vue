...
//                    that.getRecordByIndexfilter(tablename,0,0,filter);
            ...
            getTreeData: function (filter) {
                ...
                that.checkedkeys=[];
                that.$http
                    .get(that.BASE_INFO.BASE_DATA_URL+"/getStModel")
                    .then(function (response) {
                        if (
                            ...
                            response.data.result.Setting.Sites
                        ) {
                            var list = response.data.result.Setting.Sites.Net_Site;
                             that.tables= response.data.result.Setting.tables.table;
                            that.cloumns=that.setcolumns(that.tables,that.activeName);
//                            if(that.isAll){
                            var treeitem = {};
                            treeitem.label = "厂站";
                            treeitem.number = 0;
...
//                                treeitem.children.push(oneitem);
                                    searchFilter;
                                    that.getTreeNodeData(that.activeName,item.ID32,item,treeitem,that.treeData,that.idArr,that.checkedkeys,searchFilter);
