    ...
    <div>
        <el-row :gutter="20">
            ...
            <el-col :span="20"  style="height:100%;">
                ...
                <el-card v-show="activeChart=='table'">
                <el-card shadow="hover" style="margin-bottom: 1px;" v-show="addtabs">
                        <el-row style="margin-bottom: 10px;">
                            <el-button type="primary" plain @click="dialogFormVisible = true">列选择</el-button>
                            <el-button type="info" plain @click="updateTableData" v-if="false" >修改信息</el-button>
                ...
                -->
                <!--table-->
                <el-scrollbar v-bind:style="{height:displayheight+ 'px'}">
                    <el-card v-show="activeChart=='table'">
                        ...
                        <el-table :data="viewtableData"         
                                      ...
                                      :row-class-name="tableRowClassName"   
                                        ...
                                        border>
                            <!-- <el-table-column label="操作" type="selection" width="55"  v-if="addtabs" >
                            </el-table-column> -->                            
                             <!-- 添加一个多选框,控制选中与否 -->
                            ...
                            <el-table-column label="曲线" width="55" v-if="isSCADA_Analog" >
                                <template slot-scope="scope">
                    ...
                    <!--trend-->
                    <el-card  v-show="activeChart=='data'">
                        <div class="div-line" v-show="chartName=='line'" style="margin-top: 10px;" v-if="isSCADA_Analog">
                            <vcharts style="width: 100%;" v-bind:style="{height:chartheight+ 'px'}"  ref="ArchiveMore"></vcharts>
                        </div>
                    </el-card>
                </el-scrollbar>
            </el-col>
...
<script>
    import vcharts from './xd.Archivemore.vue';
    export default {
        data() {
            ...
            return {                
                displayheight:700,  //table+chart的高度
                chartheight:500,  //chart的高度
                chartName:"line",
                tableheight:600,   //table的高度
                treeheight:500,   //树的高度
        ...
        methods: {
            ...
            serach(){
                ...
                var ids=[parseInt(this.currentRecordID)];
          ...
          //      that.tableheight=height-300;
                that.treeheight=height-10;
                that.displayheight=height-150;
...
//      that.columns = hearderList;
                ...
                that.checkvalue=[];
                for(var index=0; index<6&&index<checklist.length; index++)
                {
                    that.checkvalue.push(checklist[index].key);
                }
                ...
                if (activeName === tab.name) {
                    ...
                    for( var t of this.tabList){
                        if(t.id== tabname){
                            ...
                            this.checkvalue=[];
                            for(var index=0; index<8&&index<this.checkList.length; index++)
                            {
                                this.checkvalue.push(this.checkList[index].key);
                            }
...
//                                }
                ...
                that.viewtableData.forEach(item => {
                    item.checked = false
                })
            ...
            handleSelectionChange(row) {
                ...
                this.viewtableData.forEach(item => {
                    // 排他,每次选择时把其他选项都清除
                    if (item.ID32 == row.ID32) {
                        item.checked = true;
                        this.$set(this.viewtableData, row.index, row) 
                        
                        //local display
                        let that = this;
                        if(that.isSCADA_Analog)
                        {
                            that.$refs.ArchiveMore.initDataToChart(row.ID32, this.tableName);
                        }
